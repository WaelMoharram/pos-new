...
@section('title', 'فواتير مرتجع توريد')

@section('content_header')
    <h1>فواتير مرتجع توريد</h1>
@stop

@section('content')
    {{-- Setup data for datatables --}}
    @if(request()->type == 'purchase_out')
        @can('add purchases-return')
    @component('partials.buttons._add_purchase_bill_button',[
                                            'route' => route('bills.store',['type'=>'purchase_out']) ,
                                            'tooltip' => 'اضافة',
                                            'store'=>$store
                                             ])
    @endcomponent
